...
namespace KphpParamDepends;

use Classes\A as UsedA;
use Classes\B as UsedB;

class A {
  function method(): int { echo "A method\n"; return 1; }
}
class B {
  function method(): int { echo "B method\n"; return 2; }
}
class C {
  function method(): int { echo "C method\n"; return 3; }
}
 ...
 * @return future<T>
 */
function fork($a): future { return new future($a); }
...
f1/*<\KphpParamDepends\A, \KphpParamDepends\A>*/([null], [new A]);
f1/*<B, B>*/([new B], [new B]);
f1/*<A, B>*/([new A], [new B]);
...
function get_int(int $i): int { sched_yield(); return $i; }
function get_AVal(string $val): AVal { sched_yield(); return new AVal($val); }

/**
 * @return tuple(int, Aval)
 */
function get_tuple() { sched_yield(); return tuple(-1, new AVal("-1")); }

/** @return future<int> */
function getFuture_int(int $i) { return fork(get_int($i)); }
/** @return future<AVal> */
function getFuture_AVal(string $val) { return fork(get_AVal($val)); }
